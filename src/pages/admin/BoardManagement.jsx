...
// ------------------------------------------------------------
// 📜 신고 검토 컴포넌트 (F-S-BM-005)
// ------------------------------------------------------------
const ReportReview = () => {
    const [reports, setReports] = useState(dummyReports);

    // F-S-BM-004: 신고 처리 (게시글 삭제) 더미 함수
    const handleProcess = (id) => {
        if (window.confirm("신고 내용을 검토하고 해당 게시물/댓글을 삭제 처리(F-S-BM-004) 하시겠습니까?")) {
            alert(`F-S-BM-005: ${id}번 신고 처리 완료. 관련 콘텐츠 삭제됨.`);
    ...
    return (
        <div>
            <h3>신고 게시글 검토 및 처리 (F-S-BM-005)</h3>
...
const PostList = () => (
    <div>
        <h3>게시글 목록 조회 (F-S-BM-001, 002)</h3>
        <p>전체 게시글과 댓글을 조회하고 상세 수정/삭제 링크를 제공합니다.</p>
        <button className="btn btn-sm btn-info me-2">게시글 상세 수정 (F-S-BM-003)</button>
        <button className="btn btn-sm btn-danger">게시글 삭제 (F-S-BM-004)</button>
    </div>
);

const NoticeManager = () => (
    <div>
        <h3>공지사항 등록/관리 (F-S-BM-006)</h3>
        <p>공지사항을 작성, 수정, 삭제하는 UI 틀입니다.</p>
        <input type="text" className="form-control mb-2 w-50" placeholder="공지사항 제목 작성/수정 (F-S-BM-006)" />
        <textarea className="form-control mb-2 w-75" rows="3" placeholder="공지사항 내용 작성/수정 (F-S-BM-006)"></textarea>
        ...
        <div className="board-management">
            ...
            <ul className="nav nav-tabs mb-4">
                <li className="nav-item">
                    <Link to="notice" className="nav-link">공지 관리</Link>
                </li>
                <li className="nav-item">
                    <Link to="posts" className="nav-link">전체 게시글</Link>
                </li>
                <li className="nav-item">
                    <Link to="reports" className="nav-link">신고 검토</Link>
                </li>
            </ul>
